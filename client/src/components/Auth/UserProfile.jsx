...
const UserProfile = () => {
  ...
  const stats = useMemo(() => {
    ...
    const hostedMeetings = meetings.filter(m => (m.host && (m.host._id || m.host).toString() === (userId && userId.toString()))).length;
    const joinedMeetings = meetings.filter(m => (m.participants || []).some(p => (p.user && (p.user._id || p.user).toString() === (userId && userId.toString())))).length;
    ...
    for (let i = range - 1; i >= 0; i--) {
      ...
      lastDays.push({ date: date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), count });
    }

    return { totalMeetings, hostedMeetings, joinedMeetings, activityDays: lastDays };
  ...
  const openCaptions = async (meeting) => {
    try {
      const res = await api.get(`/meetings/${meeting.meetingId}/captions`, { responseType: 'text' });
      ...
      <div className="mt-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="bg-gradient-to-br from-wwc-500 to-wwc-600 rounded-2xl shadow-medium p-6 text-white">
            ...
            <p className="text-xs opacity-75 mt-1">Meetings you hosted or joined</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-accent-500 to-accent-600 rounded-2xl shadow-medium p-6 text-white">
            ...
            <p className="text-xs opacity-75 mt-1">Meetings you joined as a participant</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-success-500 to-success-600 rounded-2xl shadow-medium p-6 text-white">
            ...
            <p className="text-xs opacity-75 mt-1">Meetings where you were the host</p>
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-medium border border-neutral-100 p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-neutral-900">Your Meeting Activity</h2>
        ...
        <div className="bg-white rounded-2xl shadow-medium border border-neutral-100 p-6">
          <h2 className="text-2xl font-bold text-wwc-700 mb-4">Your Meetings</h2>
              ...
              <tbody>
                {meetings.map((m) => (
                  <tr key={m._id} className="border-b">
                    ...
                    <td className="py-2 px-4">
                      {m.captionsTextPath ? (
                        <button onClick={() => openCaptions(m)} className="bg-wwc-600 hover:bg-wwc-700 text-white font-medium py-1 px-3 rounded-md text-sm transition-colors">View</button>
                      ) : (
