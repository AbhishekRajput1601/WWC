...
const AllUsers = ({ user, isMuted, participants }) => (
  <div className="w-[400px] h-[570px] max-h-[calc(100vh-120px)] bg-white border-2 border-black rounded-2xl shadow-2xl flex flex-col overflow-hidden mt-1 mr-5">
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-3">
      ...
      <div className="flex items-center justify-between p-3 bg-wwc-50 rounded-xl border border-wwc-200 w-full relative">
        <div className="flex items-center space-x-3">
          <div className="w-9 h-9 bg-gradient-to-br from-wwc-600 to-wwc-700 rounded-full flex items-center justify-center">
            <span className="text-white font-semibold text-base">{user?.name?.[0] || "U"}</span>
          </div>
          <div>
            <p className="font-medium text-neutral-900 text-base">{user?.name || "You"}</p>
      ...
      {participants && participants.map((p) => (
        <div key={p.socketId} className="flex items-center justify-between p-3 bg-neutral-100 rounded-xl border border-neutral-200 w-full relative">
          <div className="flex items-center space-x-3">
            <div className="w-9 h-9 bg-gradient-to-br from-wwc-400 to-wwc-600 rounded-full flex items-center justify-center">
              <span className="text-white font-semibold text-base">{p.userName?.[0] || "U"}</span>
            </div>
            <div>
              <p className="font-medium text-neutral-900 text-base">{p.userName}</p>
