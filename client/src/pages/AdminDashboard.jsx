...
const AdminDashboard = () => {
  ...
  const [addUserStatus, setAddUserStatus] = useState({}); // { meetingId_userId: status }
  ...
  useEffect(() => {
    ...
    fetchData();
  // Add user to meeting handler
  const handleAddUserToMeeting = async (meetingId, userId) => {
    setAddUserStatus(s => ({ ...s, [`${meetingId}_${userId}`]: 'loading' }));
    try {
      const res = await api.post('/meeting/add-user', { meetingId, userId });
      if (res.data.success) {
        setAddUserStatus(s => ({ ...s, [`${meetingId}_${userId}`]: 'success' }));
      } else {
        setAddUserStatus(s => ({ ...s, [`${meetingId}_${userId}`]: 'error' }));
      }
    } catch {
      setAddUserStatus(s => ({ ...s, [`${meetingId}_${userId}`]: 'error' }));
    }
    ...
    <div className="max-w-7xl mx-auto py-10 px-4">
      ...
      <h2 className="text-lg font-medium text-neutral-700 mb-2">
        ...
        <div className="bg-white rounded-2xl shadow-medium border border-neutral-100 p-6">
          <h2 className="text-2xl font-bold text-wwc-700 mb-4">All Meetings & Participants</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full text-left">
              <thead>
                <tr className="bg-neutral-100">
